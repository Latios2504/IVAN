...
public class AIQueryEngine : IAIQueryEngine
{
    ...
    public async Task<List<string>> GetRequiredTablesAsync(string queryCategory, string specificQuery)
    {
        ...
        switch (queryCategory.ToLower())
        {
            ...
            case "event count":
                tables.AddRange(new[] { "Events", "EventCategories" });
                break;
    ...
    public async Task<Dictionary<string, object>> ExtractQueryParametersAsync(string query)
    {
        ...
        if (volunteerIdMatches.Count > 0)
        {
            foreach (Match match in volunteerIdMatches)
            {
                for (int i = 1; i <= 4; i++)
                {
                    if (match.Groups[i].Success && int.TryParse(match.Groups[i].Value, out int volunteerId))
                    {
                        parameters["volunteerId"] = volunteerId;
                        _logger.LogInformation("Extracted volunteerId: {VolunteerId} from match group {GroupIndex}", volunteerId, i);
                        break;
                    }
                }
            }
        ...
        var eventIdMatches = Regex.Matches(queryLower, @"event.*?id\s*(\d+)|id\s*(\d+).*event|sự kiện.*?id\s*(\d+)|id\s*(\d+).*sự kiện");
        if (eventIdMatches.Count > 0)
        {
            foreach (Match match in eventIdMatches)
            {
                for (int i = 1; i <= 4; i++)
                {
                    if (match.Groups[i].Success && int.TryParse(match.Groups[i].Value, out int eventId))
                    {
        ...
        // Extract numbers (if not already captured as IDs)
        var numberMatches = Regex.Matches(query, @"\b(\d+)\b");
        if (numberMatches.Count > 0)
        {
            var numbers = numberMatches.Select(m => int.Parse(m.Value)).ToList();
            parameters["numbers"] = numbers;

            // If we found numbers but no specific ID, try to infer the context
            if (!parameters.ContainsKey("volunteerId") && !parameters.ContainsKey("eventId"))
            {
                if (Regex.IsMatch(queryLower, @"\b(volunteer|tình nguyện)\b"))
                {
                    parameters["volunteerId"] = numbers.First();
                }
                else if (Regex.IsMatch(queryLower, @"\b(event|sự kiện)\b"))
                {
                    parameters["eventId"] = numbers.First();
                }
            }
        }
        ...
        // Extract query type hints
        if (Regex.IsMatch(queryLower, @"\b(chi tiết|thông tin|details|info)\b"))
            parameters["queryType"] = "details";
        
        if (Regex.IsMatch(queryLower, @"\b(thống kê|statistics|stats|báo cáo|report)\b"))
            parameters["queryType"] = "analytics";

        if (Regex.IsMatch(queryLower, @"\b(danh sách|list|show|hiển thị)\b"))
            parameters["queryType"] = "list";

        // **DEBUG LOGGING FOR FINAL PARAMETERS**
        _logger.LogInformation("ExtractQueryParametersAsync final parameters: {Parameters}", 
    ...
    private bool ContainsCountingKeywords(string query)
    {
        var keywords = new[] 
        { 
            "bao nhiêu", "có bao nhiêu", "số lượng", "tổng số", "count", "total", 
            "how many", "hiện có", "đếm", "thống kê", "statistics"
        };
        return keywords.Any(query.Contains);
    }
