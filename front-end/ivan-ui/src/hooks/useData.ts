...
interface UseDataService<T, CreateType, UpdateType> {
  getAll?: () => Promise<T[]>;
...
interface UseDataReturn<T, CreateType, UpdateType> {
  // State
  data: T[];
  loading: boolean;
  error: string | null;
...
/**
 * Generic hook for data management - replaces complex Context patterns
 */
...
>(
  service: UseDataService<T, CreateType, UpdateType>,
  options: UseDataOptions = {}
): UseDataReturn<T, CreateType, UpdateType> {
  const {
    successMessages = {
  ...
  const handleError = useCallback(
    (error: unknown, action: string) => {
      ...
      console.error(`useData.${action} error:`, error);
    },
    [throwOnError]
  );
  ...
  const loadAll = useCallback(async () => {
    if (!service.getAll) {
      console.warn("useData: getAll method not provided in service");
      return;
    }
  ...
  const loadById = useCallback(
    async (id: number | string): Promise<T | null> => {
      if (!service.getById) {
        console.warn("useData: getById method not provided in service");
        return null;
      }
  ...
  const create = useCallback(
    async (createData: CreateType): Promise<T | null> => {
      if (!service.create) {
        console.warn("useData: create method not provided in service");
        return null;
      }
  ...
  const update = useCallback(
    async (id: number | string, updateData: UpdateType): Promise<T | null> => {
      if (!service.update) {
        console.warn("useData: update method not provided in service");
        return null;
      }
  ...
  const remove = useCallback(
    async (id: number | string): Promise<boolean> => {
      if (!service.delete) {
        console.warn("useData: delete method not provided in service");
        return false;
      }
