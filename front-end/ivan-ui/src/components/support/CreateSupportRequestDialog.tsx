...
export const CreateSupportRequestDialog = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.categoryId || !formData.subject.trim() || !formData.description.trim()) {
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Mô tả chi tiết vấn đề bạn gặp phải để chúng tôi có thể hỗ trợ bạn tốt nhất.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <SelectItem key={category.categoryId} value={category.categoryId.toString()}>
                    <div className="flex items-center justify-between w-full">
                      ...
                      <div className="flex items-center space-x-2 ml-2">
                        <Badge className={getPriorityColor(category.priority || 'Medium')}>
                          {category.priority === 'High' ? 'Cao' :
                           category.priority === 'Medium' ? 'Trung bình' :
                           category.priority === 'Low' ? 'Thấp' : category.priority}
                        </Badge>
                ...
                <CardContent className="p-3">
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-xs text-blue-600">
                    <div className="flex items-center space-x-1">
                      ...
                      <span>Mức độ: {selectedCategory.priority === 'High' ? 'Cao' :
                                     selectedCategory.priority === 'Medium' ? 'Trung bình' :
                                     selectedCategory.priority === 'Low' ? 'Thấp' : selectedCategory.priority}</span>
                    </div>
                    {selectedCategory.expectedResponseTime && (
                      <div className="flex items-center space-x-1">
                        <Clock className="w-3 h-3" />
                        <span>Thời gian phản hồi dự kiến: {selectedCategory.expectedResponseTime} giờ</span>
                      </div>
                    )}
                  </div>
                </CardContent>
              ...
              <SelectContent>
                <SelectItem value="Low">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600">- Không cấp thiết</span>
                  </div>
                </SelectItem>
                <SelectItem value="Medium">
                  <div className="flex items-center space-x-2">
                    <Badge className={getPriorityColor('Medium')}>Trung bình</Badge>
                    <span className="text-sm text-gray-600">- Cần giải quyết sớm</span>
                  </div>
                </SelectItem>
                <SelectItem value="High">
                  <div className="flex items-center space-x-2">
                    <Badge className={getPriorityColor('High')}>Cao</Badge>
                    <span className="text-sm text-gray-600">- Cần giải quyết ngay</span>
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-gray-500">
              Hãy cung cấp thông tin chi tiết để chúng tôi có thể hỗ trợ bạn tốt nhất.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              <p className="text-sm text-gray-600">
                Tính năng đính kèm tệp sẽ được phát triển trong phiên bản tiếp theo
              </p>
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={loading || !formData.categoryId || !formData.subject.trim() || !formData.description.trim()}
